...
function applySeedEventCorrections(events: EventSnapshot[]): EventSnapshot[] {
  const correctedById = new Map(events.map((event) => [event.id, event]));

  for (const seedEvent of seedEvents) {
    correctedById.set(seedEvent.id, seedEvent);
  }

  return getSortedEvents([...correctedById.values()]);
}
...
export function normalizeSeasonState(raw: unknown): SeasonState {
  ...
  return {
    events: applySeedEventCorrections(events.length > 0 ? events : seedEvents),
    drafts,
  };
}
